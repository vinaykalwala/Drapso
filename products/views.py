...
def wholeseller_variant_edit(request, variant_id):
    ...
    if request.method == 'POST':
        form = WholesellerVariantForm(request.POST, request.FILES, instance=variant)
        image_formset = WholesellerVariantImageFormSet(request.POST, request.FILES, instance=variant, prefix='images')

        if form.is_valid() and image_formset.is_valid():
            form.save()
            image_formset.save()
            messages.success(request, f'✅ Variant "{variant.variant_name}" updated successfully!')
            return redirect('products:wholeseller_product_variants', product_id=variant.product.id)
        else:
            messages.error(request, "❌ Please fix the errors in the form.")

    else:
        form = WholesellerVariantForm(instance=variant)
        image_formset = WholesellerVariantImageFormSet(instance=variant, prefix='images')
...
def reseller_product_list(request, store_id):
    ...
    products = (
        ...
        .filter(store=store)
        .select_related('category', 'subcategory')
        .prefetch_related('additional_images', 'variants', 'variants__additional_images')
    )
    ...
    if status_filter == 'published':
        ...
        products = products.filter(price_status__in=['price_increased', 'price_decreased'])
...
def reseller_import_products(request, store_id):
    ...
    if current_count >= max_products:
        messages.error(request, f'You have reached your product limit of {max_products}.')
...
def reseller_product_edit(request, store_id, product_id):
    ...
    if product.source_type == 'imported':
        if request.method == 'POST':
            form = ResellerImportProductEditForm(request.POST, instance=product)
            ...
            if form.is_valid():
                ...
                for v in product.variants.all():
                    ...
                    # Recalculate discounted price for variant
    ...
    if request.method == 'POST':
        ...
        image_formset = ResellerProductImageFormSet(request.POST, request.FILES, instance=product)
    ...
    if request.method == 'POST':
        ...
        if product_form.is_valid() and image_formset.is_valid() and variant_formset.is_valid():
            ...
            for i, variant in enumerate(variants):
                ...
                for img in images:
                    ResellerVariantImage.objects.create(variant=variant, image=img)
...
def reseller_product_full_edit(request, store_id, product_id):
    ...
    if request.method == 'POST':
        ...
        variant_formset = ResellerVariantFormSet(request.POST, request.FILES, instance=product, prefix='variants')
        ...
        variant_formset = ResellerVariantFormSet(instance=product, prefix='variants')
...
def calculate_price(request):
    ...
    updated_variants = [round(float(p) + margin, 2) for p in variant_prices]
